...
def process_useful_columns(df_input):
    ...
    for group in df["Group"].unique():
        ...
        full_cycling = (
            df_group["Voltage"].max() >= 0.9 * df["Voltage"].max()
            and df_group["Voltage"].min() <= 1.1 * df["Voltage"].min()
        )
        ...
        if (df_groupby_testduration.loc[group] > threshold_pulse_duration) and (len(df_group_current) != 0):
            df.loc[df["Group"] == group, "State"] = (
                "C" if df_group_current["Voltage"].iloc[-1] - df_group_current["Voltage"].iloc[0] >= 0 else "D"
            )
            ...
            if successive_pulse_end - successive_pulse_start > 200:
                df.loc[
                    (df["TestTime"] >= successive_pulse_start) & (df["TestTime"] <= successive_pulse_end), "State"
                ] = ("C" if successive_voltage_end - successive_voltage_start >= 0 else "D")
            ...
            # if it is a fast cycling (cycling that lasts less than the threshold duration), the state corresponds to the voltage difference between the start and the end of the group
                ...
                df.loc[df["Group"] == group, "State"] = (
                    "C" if df_group["Voltage"].iloc[-1] - df_group["Voltage"].iloc[0] >= 0 else "D"
                )
        else:
            df.loc[df["Group"] == group, "State"] = (
                "C" if df_group["Voltage"].iloc[-1] - df_group["Voltage"].iloc[0] >= 0 else "D"
    ...
    if successive_pulse_count > 0:
        df.loc[(df["TestTime"] >= successive_pulse_start) & (df["TestTime"] <= successive_pulse_end), "State"] = (
            "C" if successive_voltage_end - successive_voltage_start >= 0 else "D"
        )
    ...
    df["Cycle"] = 0
    df.loc[df["TestTime"] > fist_discharge_time, "Cycle"] = (
        ((df_discharge["State"] != df_discharge["State"].shift()).cumsum() + 1) // 2
    ).astype(int)
    ...
    if df["Cycle"].nunique() == 1:
        ...
        df["Capacity"] = (
            df.groupby("Cycle")
            .apply(lambda group: (group["Current"] * group["TestTime"].diff()).cumsum() / 3600)
