...
def calculate_dqdv_for_all_cycle(df, smoothing=True):
    ...
    for charging_state in ["D", "C"]:
        ...
        for cycle in df["Cycle"].unique():
            ...
            if len(df_cycle) > 3:
                df_smoothed, sigma = calculate_dqdv_for_one_cycle(df_cycle, smoothing=smoothing, sigma=sigma)
    ...
    if len(df_dqdv) > 3:
        volt_step = (
            len(df["Cycle"].unique()) * (df_dqdv["smoothed_voltage"].max() - df_dqdv["smoothed_voltage"].min()) / 50000
        )
...
def calculate_dqdv_for_one_cycle(df, smoothing=True, sigma=None):
    ...
    """
    df_smoothed = df.sort_values(by="Voltage").drop_duplicates(subset="Voltage").reset_index(drop=True).copy()
    ...
    if abs(df_smoothed["Voltage"].diff().min()) > 1e-3 and smoothing:
        ...
        df_smoothed = pd.DataFrame({"Voltage": voltage_linspace, "Capacity": f(voltage_linspace)})

    # Rounding Voltage to 1mV and taking the median Capacity values
    df_smoothed["Voltage"] = round(df_smoothed["Voltage"], 3)
    df_smoothed = df_smoothed.groupby("Voltage", as_index=False).agg({"Capacity": "median"}).reset_index(drop=True)

    df_smoothed["dqdv"] = np.gradient(df_smoothed["Capacity"], df_smoothed["Voltage"])

    df_smoothed[["dqdv"]] = df_smoothed[["dqdv"]].fillna(0)
    df_smoothed = df_smoothed.replace([np.inf, -np.inf], np.nan).dropna(subset=["dqdv"]).reset_index(drop=True)
    ...
    if smoothing:
        if sigma:
            ...
            df_smoothed["smoothed_dqdv"] = gaussian_filter1d(df_smoothed["dqdv"], sigma=sigma)
            return df_smoothed, sigma

        else:
            # Find the height and width of the largest peak
            peaks, properties = find_peaks(abs(df_smoothed["dqdv"]) / abs(df_smoothed["dqdv"]).max(), height=0.15)
            if len(peaks) > 0:
...
def create_linspace_voltage(df, voltage_step):
    ...
    for charging_state in df["State"].unique():
        for cycle in df["Cycle"].unique():
            ...
            df_cycle = df[(df["Cycle"] == cycle) & (df["State"] == charging_state)].sort_values(by="smoothed_voltage")

            if len(df_cycle) > 3:
                df_linspace_cycle = pd.DataFrame(
                    {"smoothed_voltage": linspace_voltage, "Cycle": cycle, "State": charging_state}
                )

                for var in ["smoothed_dqdv", "smoothed_capacity"]:
                    f = interp1d(
                        df_cycle["smoothed_voltage"],
                        df_cycle[var],
                        kind="linear",
                        bounds_error=False,
                        fill_value=(df_cycle[var].iloc[0], df_cycle[var].iloc[-1]),
                    )
