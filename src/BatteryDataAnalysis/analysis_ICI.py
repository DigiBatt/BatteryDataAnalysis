    ...
    *V0, V1, V2, V3, t0, t1, t2, t3, Ipulse, delta_Ec, delta_Edrop, delta_Epulse* that are calculated in :func:`calculate_relevant_points_ICI`.
    ...
    for cycle in df["Cycle"].unique():
        for state in df[df["Cycle"] == cycle]["State"].unique():
            ...
            df_group = (
                df_state[df_state["normcurrent"] == 0].groupby("Pulse")["TestTime"].agg(lambda x: x.max() - x.min())
            )
...
def global_calculation_ICI(df_nested, my_func_list):
    ...
    for pulse, df_pulse in df_nested.items():
        ...
        if len(df_pulse[df_pulse["Relaxation"] == 0]) > 2 and len(df_pulse[df_pulse["Relaxation"] == 1]) > 2:
            try:
                V0, V1, V2, V3, t0, t1, t2, t3, Ipulse, delta_Ec, delta_Edrop, delta_Epulse = (
                    calculate_relevant_points_ICI(df_pulse)
                )
...
def calculate_relevant_points_ICI(df_pulse):
    ...
    whittaker_smoother = WhittakerSmoother(
        lmbda=1e3, order=1, data_length=len(df_relax), x_input=df_relax["TestTime"], weights=weights_padded
    )
